...
describe("BOARD-1 seed specification", () => {
  ...
  it("has exactly 192 unique source obligations per year, 384 total", () => { const rows = buildSubmissionObligations(); expect(rows).toHaveLength(384); expect(new Set(rows.map(r=>r.externalKey)).size).toBe(384); expect(rows.filter(r=>r.month.startsWith("2026"))).toHaveLength(192); expect(rows.filter(r=>r.month.startsWith("2025"))).toHaveLength(192); expect(rows.every(r => !r.source.includes("category3"))).toBe(true); });
  ...
  it("builds real deterministic evidence bytes and checksums", () => { const a=buildSyntheticEvidence(), b=buildSyntheticEvidence(); expect(a).toHaveLength(8); a.forEach((f,i)=>{expect(f.bytes.length).toBeGreaterThan(100); expect(createHash("sha256").update(f.bytes).digest("hex")).toBe(f.sha256); expect(f.sha256).toBe(b[i].sha256); expect(f.bytes.toString("utf8")).toContain("Synthetic demonstration");}); });
});
