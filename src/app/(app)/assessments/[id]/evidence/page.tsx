                  ...
                  <span className="flex items-center justify-end gap-1">
                    ...
                    {canEdit && (
                      <DestructiveActionDialog
                        triggerLabel={`Remove ${item.title}`}
                        triggerIcon={<Trash2 className="h-3.5 w-3.5" />}
                        triggerVariant="ghost"
                        title={`Permanently delete "${item.title}"?`}
                        description="This is audit evidence — deleting it removes both the file and its record permanently, with no undo. It does not affect any calculation that already ran, but the supporting document behind that calculation will be gone."
