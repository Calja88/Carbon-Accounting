...
import {
  createMonitoringEquipmentAction, createMonitoringPlanAction, deactivateMonitoringPlanAction,
  documentMonitoringExceptionAction, notifyOverdueCalibrationsAction, recordEquipmentCalibrationAction,
  recordMonitoringResultAction, retireMonitoringEquipmentAction, reviewMonitoringResultAction,
  uploadCalibrationCertificateAction, uploadMonitoringResultEvidenceAction,
...
type PlanRow = {
  ...
  acceptanceCriteria: string; status: string; aspectId: string | null; aspectLabel: string | null;
  controlId: string | null; controlLabel: string | null;
...
function ResultItem({ result, canRecord, canReview }: { result: ResultRow; canRecord: boolean; canReview: boolean }) {
  ...
  return <div className="space-y-2 rounded border border-slate-200 p-3 text-sm">
    ...
    {canReview && result.reviewStatus === "PENDING" && <form action={reviewAction} className="flex flex-wrap items-end gap-2"><input type="hidden" name="resultId" value={result.id} /><div className="min-w-64 flex-1"><Label htmlFor={`reviewNote-${result.id}`}>Review note</Label><Input id={`reviewNote-${result.id}`} name="reviewNote" required /></div><Button type="submit" variant="secondary" disabled={reviewing}>{reviewing ? "Reviewing…" : "Review result"}</Button><Feedback state={reviewState} /></form>}
    {canRecord && <form action={evidenceAction} className="flex flex-wrap items-end gap-2"><input type="hidden" name="resultId" value={result.id} /><div><Label htmlFor={`resultEvidence-${result.id}`}>Result evidence</Label><Input id={`resultEvidence-${result.id}`} name="file" type="file" required /></div><Input name="purpose" placeholder="Purpose (optional)" /><Button type="submit" variant="secondary" disabled={uploading}>{uploading ? "Uploading…" : "Attach"}</Button><Feedback state={evidenceState} /></form>}
  </div>;
}

function DeactivatePlanForm({ planId }: { planId: string }) {
  const [state, action, pending] = useActionState(deactivateMonitoringPlanAction, emptyState);
  return <form action={action} className="flex flex-wrap items-end gap-2 rounded border border-slate-200 p-3">
    <input type="hidden" name="planId" value={planId} />
    <div className="min-w-64 flex-1"><Label htmlFor={`deactivateReason-${planId}`}>Deactivation reason</Label><Input id={`deactivateReason-${planId}`} name="reason" required /></div>
    <Button type="submit" variant="danger" disabled={pending} onClick={(event) => { if (!confirm("Deactivate this monitoring plan? Recorded results are kept, but no new results can be appended.")) event.preventDefault(); }}>{pending ? "Deactivating…" : "Deactivate plan"}</Button>
    <Feedback state={state} />
  </form>;
}

function RetireEquipmentForm({ equipmentId }: { equipmentId: string }) {
  const [state, action, pending] = useActionState(retireMonitoringEquipmentAction, emptyState);
  return <form action={action} className="flex flex-wrap items-end gap-2 rounded border border-slate-200 p-3">
    <input type="hidden" name="equipmentId" value={equipmentId} />
    <div><Label htmlFor={`retireStatus-${equipmentId}`}>Move to</Label><Select id={`retireStatus-${equipmentId}`} name="status" defaultValue="RETIRED"><option value="RETIRED">Retired</option><option value="OUT_OF_SERVICE">Out of service</option></Select></div>
    <div className="min-w-64 flex-1"><Label htmlFor={`retireReason-${equipmentId}`}>Reason</Label><Input id={`retireReason-${equipmentId}`} name="reason" required /></div>
    <Button type="submit" variant="danger" disabled={pending} onClick={(event) => { if (!confirm("Take this equipment out of use? Its calibration history is kept.")) event.preventDefault(); }}>{pending ? "Updating…" : "Take out of use"}</Button>
    <Feedback state={state} />
  </form>;
}
...
function PlanCard({ plan, canRecord, canReview }: { plan: PlanRow; canRecord: boolean; canReview: boolean }) {
  ...
  return <Card><CardContent id={`plan-${plan.id}`} className="scroll-mt-24 space-y-4 py-5">
    ...
    <div className="grid gap-2 text-sm sm:grid-cols-2"><p><strong>Method:</strong> {plan.method}</p><p><strong>Location:</strong> {plan.location}</p><p><strong>Frequency:</strong> {plan.frequency}</p><p><strong>Unit:</strong> {plan.unit}</p><p><strong>Responsible:</strong> {plan.responsibleName}</p><p><strong>Instrument:</strong> {plan.equipmentLabel ?? "Not required"}</p>{plan.aspectLabel && <p><strong>Aspect:</strong> {plan.aspectId ? <a href={`/ems/aspects#aspect-${plan.aspectId}`} className="text-blue-700 underline">{plan.aspectLabel}</a> : plan.aspectLabel}</p>}{plan.controlLabel && <p><strong>Control:</strong> {plan.controlId ? <a href={`/ems/controls#control-${plan.controlId}`} className="text-blue-700 underline">{plan.controlLabel}</a> : plan.controlLabel}</p>}<p className="sm:col-span-2"><strong>Criteria:</strong> {plan.acceptanceCriteria}</p></div>
    {canRecord && plan.status !== "INACTIVE" && <DeactivatePlanForm planId={plan.id} />}
    {canRecord && plan.status === "ACTIVE" && <form action={action} className="space-y-3 rounded-lg border border-slate-200 p-3"><input type="hidden" name="planId" value={plan.id} /><input type="hidden" name="unit" value={plan.unit} /><p className="text-sm font-medium">Append monitoring result</p><div className="grid gap-3 sm:grid-cols-4"><div><Label htmlFor={`measuredAt-${plan.id}`}>Measured</Label><Input id={`measuredAt-${plan.id}`} name="measuredAt" type="date" required /></div><div><Label htmlFor={`value-${plan.id}`}>Value ({plan.unit})</Label><Input id={`value-${plan.id}`} name="value" inputMode="decimal" required /></div><div><Label htmlFor={`dataQualityFlag-${plan.id}`}>Quality</Label><Select id={`dataQualityFlag-${plan.id}`} name="dataQualityFlag" defaultValue="VALIDATED"><option value="VALIDATED">Validated</option><option value="ESTIMATED">Estimated</option><option value="SUSPECT">Suspect</option><option value="REJECTED">Rejected</option></Select></div><div><Label htmlFor={`qualitativeResult-${plan.id}`}>Qualitative result</Label><Input id={`qualitativeResult-${plan.id}`} name="qualitativeResult" /></div></div><div className="grid gap-3 sm:grid-cols-2"><div><Label htmlFor={`periodStart-${plan.id}`}>Period start (optional)</Label><Input id={`periodStart-${plan.id}`} name="periodStart" type="date" /></div><div><Label htmlFor={`periodEnd-${plan.id}`}>Period end (optional)</Label><Input id={`periodEnd-${plan.id}`} name="periodEnd" type="date" /></div></div><Feedback state={state} /><Button type="submit" disabled={pending}>{pending ? "Recording…" : "Append result"}</Button></form>}
...
function EquipmentCard({ equipment, canRecord, canReview }: { equipment: EquipmentRow; canRecord: boolean; canReview: boolean }) {
  ...
  return <Card><CardContent id={`equipment-${equipment.id}`} className="scroll-mt-24 space-y-4 py-5"><div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold">{equipment.reference}</h3><Badge tone={equipment.status === "ACTIVE" ? "success" : "neutral"}>{equipment.status.toLowerCase().replaceAll("_", " ")}</Badge>{equipment.calibrationOverdue && <Badge tone="danger">calibration overdue</Badge>}</div><p className="text-sm text-slate-600">{equipment.description} · {equipment.location} · owner {equipment.ownerName} · {equipment.calibrationFrequency}</p>{canRecord && equipment.status !== "RETIRED" && <form action={action} className="space-y-3 rounded border border-slate-200 p-3"><input type="hidden" name="equipmentId" value={equipment.id} /><p className="text-sm font-medium">Append calibration or verification</p><div className="grid gap-3 sm:grid-cols-3"><div><Label htmlFor={`dueDate-${equipment.id}`}>Due</Label><Input id={`dueDate-${equipment.id}`} name="dueDate" type="date" required defaultValue={equipment.calibrationDueDate.slice(0, 10)} /></div><div><Label htmlFor={`performedAt-${equipment.id}`}>Performed</Label><Input id={`performedAt-${equipment.id}`} name="performedAt" type="date" required /></div><div><Label htmlFor={`nextDueDate-${equipment.id}`}>Next due</Label><Input id={`nextDueDate-${equipment.id}`} name="nextDueDate" type="date" required /></div><div><Label htmlFor={`provider-${equipment.id}`}>Provider</Label><Input id={`provider-${equipment.id}`} name="provider" required /></div><div><Label htmlFor={`method-${equipment.id}`}>Method</Label><Input id={`method-${equipment.id}`} name="method" required /></div><div><Label htmlFor={`result-${equipment.id}`}>Result</Label><Select id={`result-${equipment.id}`} name="result" defaultValue="PASS"><option value="PASS">Pass</option><option value="FAIL">Fail</option><option value="OUT_OF_TOLERANCE">Out of tolerance</option></Select></div></div><Input name="responseReference" placeholder="Response/action reference (optional; review still required for out of tolerance)" /><Feedback state={state} /><Button type="submit" disabled={pending}>{pending ? "Recording…" : "Append calibration"}</Button></form>}{canRecord && equipment.status !== "RETIRED" && <RetireEquipmentForm equipmentId={equipment.id} />}<div className="space-y-2">{equipment.calibrations.length === 0 ? <p className="text-sm text-slate-500">No calibration records.</p> : equipment.calibrations.map((item) => <CalibrationItem key={item.id} calibration={item} canRecord={canRecord} canReview={canReview} />)}</div></CardContent></Card>;
}
...
export function MonitoringWorkspace({ plans, equipment, aspects, controls, members, canRecord, canReview }: { plans: PlanRow[]; equipment: EquipmentRow[]; aspects: Option[]; controls: Option[]; members: Option[]; canRecord: boolean; canReview: boolean }) {
  ...
  return <div className="space-y-8">{canRecord && <div className="grid gap-6 xl:grid-cols-2"><Card><CardHeader><CardTitle>Add monitoring equipment</CardTitle></CardHeader><CardContent><form action={equipmentAction} className="space-y-3"><div className="grid gap-3 sm:grid-cols-2"><div><Label htmlFor="new-equipment-reference">Reference</Label><Input id="new-equipment-reference" name="reference" required placeholder="synthetic-meter-01" /></div><div><Label htmlFor="new-equipment-owner">Owner</Label><Select id="new-equipment-owner" name="ownerMembershipId" required><option value="">Select…</option>{members.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</Select></div><div><Label htmlFor="new-equipment-description">Description</Label><Input id="new-equipment-description" name="description" required /></div><div><Label htmlFor="new-equipment-location">Location</Label><Input id="new-equipment-location" name="location" required /></div><div><Label htmlFor="new-equipment-calibrationFrequency">Calibration frequency</Label><Input id="new-equipment-calibrationFrequency" name="calibrationFrequency" required placeholder="Every 12 months" /></div><div><Label htmlFor="new-equipment-calibrationDueDate">Calibration due</Label><Input id="new-equipment-calibrationDueDate" name="calibrationDueDate" type="date" required /></div></div><Feedback state={equipmentState} /><Button type="submit" disabled={equipmentPending}>{equipmentPending ? "Adding…" : "Add equipment"}</Button></form></CardContent></Card><Card><CardHeader><CardTitle>Add monitoring plan</CardTitle></CardHeader><CardContent><form action={planAction} className="space-y-3"><div className="grid gap-3 sm:grid-cols-2"><div><Label htmlFor="new-plan-planKey">Plan key</Label><Input id="new-plan-planKey" name="planKey" required placeholder="synthetic-plan-01" /></div><div><Label htmlFor="new-plan-parameter">Parameter</Label><Input id="new-plan-parameter" name="parameter" required /></div><div><Label htmlFor="new-plan-method">Method</Label><Input id="new-plan-method" name="method" required /></div><div><Label htmlFor="new-plan-location">Location</Label><Input id="new-plan-location" name="location" required /></div><div><Label htmlFor="new-plan-frequency">Frequency</Label><Input id="new-plan-frequency" name="frequency" required /></div><div><Label htmlFor="new-plan-unit">Explicit unit</Label><Input id="new-plan-unit" name="unit" required /></div><div><Label htmlFor="new-plan-aspectId">Aspect</Label><Select id="new-plan-aspectId" name="aspectId"><option value="">None</option>{aspects.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</Select></div><div><Label htmlFor="new-plan-controlId">Operational control</Label><Select id="new-plan-controlId" name="controlId"><option value="">None</option>{controls.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</Select></div><div><Label htmlFor="new-plan-responsibleMembershipId">Responsible</Label><Select id="new-plan-responsibleMembershipId" name="responsibleMembershipId" required><option value="">Select…</option>{members.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</Select></div><div><Label htmlFor="new-plan-reviewDueDate">Review due (optional)</Label><Input id="new-plan-reviewDueDate" name="reviewDueDate" type="date" /></div><div><Label htmlFor="new-plan-obligationReference">Obligation reference (optional)</Label><Input id="new-plan-obligationReference" name="obligationReference" /></div><div><Label htmlFor="new-plan-objectiveReference">Objective reference (optional)</Label><Input id="new-plan-objectiveReference" name="objectiveReference" /></div><div className="sm:col-span-2"><Label htmlFor="new-plan-acceptanceCriteria">Acceptance criteria</Label><Textarea id="new-plan-acceptanceCriteria" name="acceptanceCriteria" required rows={2} /></div><label className="flex items-center gap-2 text-sm"><input type="checkbox" name="instrumentRequired" /> Instrument required</label><div><Label htmlFor="new-plan-equipmentId">Required equipment</Label><Select id="new-plan-equipmentId" name="equipmentId"><option value="">None</option>{equipment.map((item) => <option key={item.id} value={item.id}>{item.reference}</option>)}</Select></div></div><Feedback state={planState} /><Button type="submit" disabled={planPending}>{planPending ? "Creating…" : "Create plan"}</Button></form></CardContent></Card></div>}
