...
function ActionCard({ action, members }: { action: ActionItemRow; members: Option[] }) {
...
function ProgrammeCard({ programme, members }: { programme: ActionProgrammeRow; members: Option[] }) {
      ...
      <CardContent className="space-y-4 p-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p className="text-xs text-slate-500">Owner: {programme.ownerName}</p>
          </div>
        ...
        <div className="space-y-3">
          ...
          {programme.actions.map((action) => (
            <ActionCard key={action.id} action={action} members={members} />
          ))}
        </div>
      </CardContent>
    </Card>
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        ...
        {programmes.map((programme) => (
          <ProgrammeCard key={programme.id} programme={programme} members={members} />
        ))}
      </div>
    </div>
  );
