  ...
  return (
    <form action={action} className="space-y-4 rounded-lg border border-slate-200 p-4">
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <Label>Name</Label>
          <Input name="name" required placeholder="Synthetic example: 2026 Internal Audit Programme" />
        </div>
        <div>
          <Label>Owner</Label>
      ...
      <div>
        ...
        <Textarea name="riskBasis" rows={2} required placeholder="Why this programme prioritises what it does — synthetic example only." />
      </div>
      <div>
        <Label>Objectives (optional)</Label>
        <Textarea name="objectives" rows={2} placeholder="Synthetic example only." />
      </div>
      <div>
  ...
  return (
    <form action={action} className="space-y-3 rounded-md border border-slate-200 p-3">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div>
          <Label>Title</Label>
          <Input name="title" required placeholder="Synthetic example: North site audit" />
        </div>
        <div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div>
          <Label>Entities in scope</Label>
          <select name="entityIds" multiple size={3} className="w-full rounded-md border border-slate-300 text-sm">
            {entities.map((entity) => <option key={entity.id} value={entity.id}>{entity.name}</option>)}
          </select>
        </div>
        <div>
          <Label>Sites in scope</Label>
          <select name="siteIds" multiple size={3} className="w-full rounded-md border border-slate-300 text-sm">
      ...
      <div>
        ...
        <Textarea name="rationale" rows={2} placeholder="Synthetic example only." />
      </div>
      <Feedback state={state} />
  ...
  return (
    <form action={action} className="space-y-3 rounded-md border border-slate-200 p-3">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div>
          <Label>Title</Label>
          <Input name="title" required placeholder="Synthetic example: Site North internal audit" />
        </div>
        <div>
          <Label>Type</Label>
        ...
        <div>
          <Label>Planned audit (optional)</Label>
          <Select name="programmeItemId" defaultValue="">
        ...
        <div>
          ...
          <Input name="scheduledStart" type="date" required />
        </div>
        <div>
          <Label>Scheduled end</Label>
          <Input name="scheduledEnd" type="date" required />
        </div>
      </div>
      <div>
        <Label>Criteria summary</Label>
        <Textarea name="criteriaSummary" rows={2} required placeholder="Synthetic example: ISO 14001:2015 clause 9.2, controlled procedure PROC-04." />
      </div>
      <div>
        <Label>Objectives (optional)</Label>
        <Textarea name="objectives" rows={2} placeholder="Synthetic example only." />
  ...
  return (
    <form action={action} className="flex flex-wrap items-end gap-2">
      ...
      <div>
        ...
        <Input name="reason" placeholder="Synthetic example only." />
      </div>
...
function AssignTeamMemberForm({ auditId, members }: { auditId: string; members: Option[] }) {
  ...
  return (
    <form action={action} className="space-y-2 rounded-md border border-dashed border-slate-300 p-3">
      ...
      <div>
        <Label>Conflict notes (required if a conflict is declared)</Label>
        <Textarea name="conflictNotes" rows={2} placeholder="Synthetic example only." />
      </div>
