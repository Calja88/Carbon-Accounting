...
function ControlFields({ control, aspects, members, documents }: {
  control?: ControlRow;
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2">
      <div><Label>Control key</Label><Input name="controlKey" required readOnly={Boolean(control)} defaultValue={control?.controlKey} placeholder="synthetic-control-01" /></div>
      <div><Label>Title</Label><Input name="title" required defaultValue={control?.title} placeholder="Synthetic material handling control" /></div>
      <div><Label>Control type</Label><Select name="type" defaultValue={control?.type ?? "PROCEDURAL"}>{CONTROL_TYPES.map((type) => <option key={type} value={type}>{type.toLowerCase()}</option>)}</Select></div>
      <div><Label>Responsible owner</Label><Select name="ownerMembershipId" required defaultValue={control?.ownerMembershipId ?? ""}><option value="">Select…</option>{members.map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</Select></div>
      <div><Label>Frequency</Label><Input name="frequency" required defaultValue={control?.frequency ?? ""} placeholder="Before each synthetic run" /></div>
      <div><Label>Review due</Label><Input name="reviewDueDate" type="date" required defaultValue={control ? dateInputValue(control.reviewDueDate) : ""} /></div>
      <div className="sm:col-span-2"><Label>Linked aspects (Ctrl/Cmd to select multiple)</Label><Select name="aspectIds" multiple required defaultValue={control?.aspectIds ?? []} className="min-h-28">{aspects.map((aspect) => <option key={aspect.id} value={aspect.id}>{aspect.name} — {aspect.processName}</option>)}</Select></div>
      <div><Label>Process applicability (optional)</Label><Select name="applicabilityProcessId" defaultValue={control?.applicabilityProcessId ?? ""}><option value="">Covered by linked aspects</option>{processOptions.map((process) => <option key={process.id} value={process.id}>{process.name}</option>)}</Select></div>
      <div><Label>External-provider reference (optional)</Label><Input name="externalProviderReference" defaultValue={control?.externalProviderReference ?? ""} placeholder="Synthetic provider reference only" /></div>
      <div className="sm:col-span-2"><Label>Controlled procedure revision (optional)</Label><Select name="controlledDocumentRevisionId" defaultValue={control?.controlledDocumentRevisionId ?? ""}><option value="">No controlled-document revision</option>{documents.map((document) => <option key={document.id} value={document.id}>{document.label}</option>)}</Select></div>
      <div className="sm:col-span-2"><Label>Description</Label><Textarea name="description" rows={2} defaultValue={control?.description ?? ""} /></div>
      <div><Label>Acceptance criteria</Label><Textarea name="acceptanceCriteria" rows={3} required defaultValue={control?.acceptanceCriteria ?? ""} /></div>
      <div><Label>Effectiveness criteria</Label><Textarea name="effectivenessCriteria" rows={3} required defaultValue={control?.effectivenessCriteria ?? ""} /></div>
    </div>
  );
}
...
function CreateControl({ aspects, members, documents }: { aspects: AspectOption[]; members: MemberOption[]; documents: DocumentOption[] }) {
  ...
  return <Card><CardHeader><CardTitle>Add operational control</CardTitle></CardHeader><CardContent><form action={action} className="space-y-4"><ControlFields aspects={aspects} members={members} documents={documents} /><Feedback state={state} /><Button type="submit" disabled={pending || aspects.length === 0 || members.length === 0}>{pending ? "Creating…" : "Create control"}</Button></form></CardContent></Card>;
}
...
function CheckForm({ controlId }: { controlId: string }) {
  ...
  return (
    <form action={action} className="space-y-3 rounded-lg border border-slate-200 p-3">
      ...
      <div className="grid gap-3 sm:grid-cols-3"><div><Label>Scheduled</Label><Input name="scheduledAt" type="date" required /></div><div><Label>Performed</Label><Input name="performedAt" type="date" /></div><div><Label>Result</Label><Select name="result" defaultValue="PASS"><option value="PENDING">Pending</option><option value="PASS">Pass</option><option value="FAIL">Fail</option><option value="NOT_APPLICABLE">Not applicable</option></Select></div></div>
      <div><Label>Notes</Label><Input name="notes" /></div>
      <div className="grid gap-3 sm:grid-cols-2"><div><Label>Exception summary (required for fail)</Label><Input name="exceptionSummary" /></div><div><Label>Action reference</Label><Input name="actionReference" placeholder="Explicit reference; no action is auto-created" /></div></div>
...
function ControlCard({ control, aspects, members, documents, canManage }: { control: ControlRow; aspects: AspectOption[]; members: MemberOption[]; documents: DocumentOption[]; canManage: boolean }) {
  ...
  return <Card><CardContent className="space-y-4 py-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold text-slate-900">{control.title}</h3><Badge>{control.controlKey} v{control.version}</Badge><Badge tone={control.status === "ACTIVE" ? "success" : "neutral"}>{control.status.toLowerCase()}</Badge>{overdue && <Badge tone="danger">review overdue</Badge>}</div><p className="mt-1 text-sm text-slate-600">{control.type.toLowerCase()} · owner {control.ownerName} · review {new Date(control.reviewDueDate).toLocaleDateString()}</p></div></div>{control.description && <p className="text-sm text-slate-600">{control.description}</p>}<div className="grid gap-3 text-sm sm:grid-cols-2"><p><strong>Aspects:</strong> {control.aspects.map((aspect) => aspect.name).join(", ")}</p><p><strong>Frequency:</strong> {control.frequency || "Not recorded"}</p><p><strong>Acceptance:</strong> {control.acceptanceCriteria || "Not recorded"}</p><p><strong>Effectiveness:</strong> {control.effectivenessCriteria || "Not recorded"}</p>{control.documentLabel && <p><strong>Procedure:</strong> {control.documentLabel}</p>}{control.externalProviderReference && <p><strong>Provider reference:</strong> {control.externalProviderReference}</p>}</div><CheckHistory checks={control.checks} canManage={canManage} />{canManage && control.status === "ACTIVE" && <><CheckForm controlId={control.id} /><details className="rounded-lg border border-slate-200 p-3"><summary className="cursor-pointer text-sm font-medium">Review and create successor version</summary><form action={reviewAction} className="mt-4 space-y-4"><input type="hidden" name="controlId" value={control.id} /><ControlFields control={control} aspects={aspects} members={members} documents={documents} /><Feedback state={reviewState} /><Button type="submit" disabled={reviewing}>{reviewing ? "Recording review…" : "Create reviewed version"}</Button></form></details><form action={retireAction} onSubmit={(event) => { if (!window.confirm("Retire this control? Its history and checks will remain available.")) event.preventDefault(); }}><input type="hidden" name="controlId" value={control.id} /><Button type="submit" variant="secondary" disabled={retiring}>{retiring ? "Retiring…" : "Retire control"}</Button><Feedback state={retireState} /></form></>}</CardContent></Card>;
}
