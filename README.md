...
### Real DESNZ 2026 import (post-v2)

The live factor sets now loaded are real imports, not placeholders:

- **Scope 1, Scope 2, and Cat 3/6/7 (Scope 3)**: the official **"Greenhouse gas reporting: conversion factors 2026"** publication (published 11 June 2026, updated flat-format file July 2026) — [gov.uk](https://www.gov.uk/government/publications/greenhouse-gas-reporting-conversion-factors-2026), flat-format file `ghg-conversion-factors-2026-flat-format-revised.xlsx`. Every value's exact source row (Scope/Level 1-4/UOM) is recorded in `data/defra-desnz-2026-import.csv`, which is the file actually run through the import mechanism — nothing in it was hand-typed from memory.
- **Cat 1 (purchased goods & services)**: Defra/University of Leeds' spend-based multipliers, in `data/defra-spend-based-eeio-2023-import.csv` — see Assumptions #29-30 for source and mapping detail.

Both were committed via `commitFactorImport` (the same function the admin upload UI calls) run from a script rather than clicked through the browser, since this sandbox can only reach the database over the same HTTPS-only path used for seeding — not because the UI doesn't work. The previous placeholder Scope 1/2 set was superseded (its `effectiveTo` set to the day before the new set's `effectiveFrom`), not edited or deleted, so any report generated against 2024/2025 activity data still reproduces against the placeholder figures it was actually calculated with. A few further judgment calls surfaced while doing this real import, flagged the same way as everything above:
