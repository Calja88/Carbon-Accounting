...
## Checkpoint A handoff generation — two narrow H00 tooling exceptions
...
Regenerating afterward surfaced a second, unrelated false positive: `src/app/invite/[token]/actions.ts`'s `tokenHash = hashInvitationToken(...)` — assigning the result of the local SHA-256 helper (no embedded secret) to a variable named `tokenHash`, flagged only because the variable name contains "token". Verified benign by reading the whole file and the hashing helper; fixed with the same exact-match-allowlist mechanism, one more entry in `REVIEWED_SAFE_CREDENTIAL_MATCHES`. A literal `tokenHash = "..."`/`token = "..."`/`password = "..."` assignment in that same file still fails the scan. 11 new tests total (both exceptions) in `src/lib/__tests__/handoff-secret-scan.test.ts` prove the allow/still-fail/no-leak behaviour for each.
